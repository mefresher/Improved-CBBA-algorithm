...
A1 = 0;
A1mm=[10 0;10 0]; %(xmin xmax;ymin ymax)
A1task=[];
for i = 1:100
    if AA(i,3) == 1
        A1 = A1 + 1;
...
%     agents(n).y    = rand(1)*(WORLD.YMAX - WORLD.YMIN) + WORLD.YMIN;%初始位置y
    agents(n).x = Dsum2(1,1);
    agents(n).y = Dsum2(1,2);
...
for m=1:M
    tasks(m) = task_rescue;
    tasks(m).id       = m;
    tasks(m).start    = A1task(m,4);%任务开始时间
    tasks(m).end      = A1task(m,5);%任务结束时间
    tasks(m).x        = A1task(m,1);%任务位置
    tasks(m).y        = A1task(m,2);%任务位置
    tasks(m).z        = 0;
end


%---------------------------------------------------------------------%
% Initialize communication graph and diameter  c n nm 67    
%---------------------------------------------------------------------%
...
Total_Score
saveas(1,'sch_g1.eps', 'epsc');
saveas(2,'path_g1.eps', 'epsc');
