...
close all; clear all;
addpath(genpath(cd));
addpath(genpath(genpath(cd)));

% profile on
AA = readmatrix('cluster.xlsx');
Dsum2 = readmatrix('center.xlsx');
A1 = 0; A2 = 0; A3 = 0; A4 = 0;
A1mm=[10 0;10 0]; A2mm=[10 0;10 0]; A3mm=[10 0;10 0]; A4mm=[10 0;10 0]; %(xmin xmax;ymin ymax)
A1task=[]; A2task=[]; A3task=[]; A4task=[];
for i = 1:100
    if AA(i,3) == 1
        A1 = A1 + 1;
        A1task = [A1task;AA(i,:)];
