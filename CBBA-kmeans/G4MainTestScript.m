...
A4 = 0;
A4mm=[10 0;10 0]; %(xmin xmax;ymin ymax)
A4task=[];
for i = 1:100
    if AA(i,3) == 4
        A4 = A4 + 1;
...
%     agents(n).y    = rand(1)*(WORLD.YMAX - WORLD.YMIN) + WORLD.YMIN;%初始位置y
    agents(n).x = Dsum2(4,1);
    agents(n).y = Dsum2(4,2);
...
for m=1:M,
    tasks(m) = task_rescue;
    tasks(m).id       = m;
    tasks(m).start    = A4task(m,4);%任务开始时间
    tasks(m).end      = A4task(m,5);%任务结束时间
    tasks(m).x        = A4task(m,1);%任务位置
    tasks(m).y        = A4task(m,2);
    tasks(m).z        = 0;
end
...
saveas(1,'sch_g4.eps', 'epsc');
saveas(2,'path_g4.eps', 'epsc');
