...
A2 = 0;
A2mm=[10 0;10 0];%(xmin xmax;ymin ymax)
A2task=[];
for i = 1:100
    if AA(i,3) == 2
        A2 = A2 + 1;
...
%     agents(n).y    = rand(1)*(WORLD.YMAX - WORLD.YMIN) + WORLD.YMIN;%初始位置y
    agents(n).x = Dsum2(2,1);
    agents(n).y = Dsum2(2,2);
...
for m=1:M,
    tasks(m) = task_rescue;
    tasks(m).id       = m;
    tasks(m).start    = A2task(m,4);%任务开始时间
    tasks(m).end      = A2task(m,5);%任务结束时间
    tasks(m).x        = A2task(m,1);%任务位置
    tasks(m).y        = A2task(m,2);
    tasks(m).z        = 0;
end
...
saveas(1,'sch_g2.eps', 'epsc');
saveas(2,'path_g2.eps', 'epsc');
